...
class DeliveryService {
  ...
  Future<List<DeliveryOrder>> getOrders({int page = 1}) async {
    try {
      ...
      final orders = ordersData.map((json) => DeliveryOrder.fromJson(json)).toList();
  ...
  Future<List<DeliveryOrder>> getOrdersByStatus(DeliveryStatus status, {int page = 1}) async {
    try {
      final response = await _apiClient.get(
        '/get-orders',
        queryParameters: {
  ...
  List<DeliveryOrder> _getMockOrders() {
    return [
      DeliveryOrder(
        ...
        items: [
          OrderItem(name: 'جهاز تابلت سامسونج', quantity: 5, price: 1200, productId: 1),
          OrderItem(name: 'بطارية خارجية', quantity: 10, price: 150, productId: 2),
        ],
      ...
      DeliveryOrder(
        ...
        items: [
          OrderItem(name: 'جهاز لابتوب HP', quantity: 3, price: 3500, productId: 3),
      ...
      DeliveryOrder(
        ...
        actualDelivery: DateTime.now().subtract(const Duration(days: 6)),
        items: [
          OrderItem(name: 'أجهزة تابلت تعليمية', quantity: 20, price: 900, productId: 5),
      ...
      DeliveryOrder(
        ...
        items: [
          OrderItem(name: 'أجهزة راوتر', quantity: 10, price: 250, productId: 7),
        ],
  ...
  Future<List<DeliveryOrder>> getActiveOrders({int page = 1}) async {
    try {
      ...
      return allOrders
          .where((order) =>
              order.status == DeliveryStatus.preparing || order.status == DeliveryStatus.inProgress)
          .toList();
    } catch (e) {
  ...
  Future<List<DeliveryOrder>> getCompletedOrders({int page = 1}) async {
    try {
      final allOrders = await getOrders(page: page);
      return allOrders.where((order) => order.status == DeliveryStatus.delivered).toList();
    } catch (e) {
      print('Error getting completed orders: $e');
      return _getMockOrders().where((order) => order.status == DeliveryStatus.delivered).toList();
    }
  }
  ...
  Future<List<DeliveryOrder>> getCancelledOrders({int page = 1}) async {
    try {
      final allOrders = await getOrders(page: page);
      return allOrders.where((order) => order.status == DeliveryStatus.cancelled).toList();
    } catch (e) {
      print('Error getting cancelled orders: $e');
      return _getMockOrders().where((order) => order.status == DeliveryStatus.cancelled).toList();
    }
  }
}
